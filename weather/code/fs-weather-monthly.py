...
def getMonthlyData(stationID, year, month, prov, name, flagPC):
    ...
    try:
        ...
        df = pd.DataFrame(lst, columns= oCols) ## 25columns converted!!    
        ## Filterout by year
...
monthly_stations.to_csv('h_stations.csv') ## saving stations data with hourly data
monthly_stations.head()
...
for index, row in monthly_stations.iterrows():
    print (row["StationID"])
    print (row["Province"])
    print (row["Name"]) 
    print (row['Year End'])
    
    sd = 'Jan' + row['Year End']; print("sd=", sd)
    ed = 'Dec' + row['Year End']; print("ed=", ed)
    
    start_date = datetime.strptime(sd, '%b%Y')
    end_date = datetime.strptime(ed, '%b%Y')
    
    flagPC = True  ; print("at genOutput flagPC=", flagPC)
    
    ##for dt in rrule.rrule(rrule.MONTHLY, dtstart=start_date, until=end_date):
    ##     print("getMonthlyData with parms=", row['StationID'], dt.year, dt.month, row['Province'], row['Name'], flagPC)
    df, flagPC = getMonthlyData(row['StationID'], dt.year, dt.month, row['Province'], row['Name'], flagPC)
